...
void ConfigManager::LoadVars() {
    table_t table = LuaTable(*this, "gl_version").table;

    auto get_int_index = [&](int i) { return table[LuaValue{LUA_TNUMBER, static_cast<double>(i + 1)}].value; };
    gl_version = {any_cast<LUA_NUMBER>(get_int_index(0)), any_cast<LUA_NUMBER>(get_int_index(1)),
                  any_cast<bool>(get_int_index(2))};
